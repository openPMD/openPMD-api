...
if __name__ == "__main__":
    ...
    series = io.Series("../samples/3_write_thetaMode_serial_py.h5", io.Access.create)
    ...
    E_r_data = np.arange(num_fields * N_r * N_z, dtype=np.double).reshape(
        num_fields, N_r, N_z
    )
    E_t_data = np.arange(num_fields * N_r * N_z, dtype=np.single).reshape(
        num_fields, N_r, N_z
    )
    ...
    E.axis_labels = ["r", "z"]
    E.data_order = "C"
    E.unit_dimension = {io.Unit_Dimension.I: 1.0, io.Unit_Dimension.J: 2.0}
