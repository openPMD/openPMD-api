...
public:
  ...
  fullscan( Series& series, MeshRecordComponent& rho )
  {
    ...
    if ( (grid[0] * grid[1] > m_MPISize) || (m_MPISize % (grid[0]*grid[1]) != 0) )
    {
      if ( 0 == m_MPIRank ) 
  ...
  block(Series& series, MeshRecordComponent& rho)
  {
    ...
    int  alongDim     = m_Pattern/10 % 10;    

    int fractionOnDim = m_Pattern/100;

    bool atCenter      = ( (m_Pattern % 3 == 0) || (fractionOnDim == 1) );
    bool atTopLeft     = ( (m_Pattern % 3 == 1) && (fractionOnDim > 1) );
    bool atBottomRight = ( (m_Pattern % 3 == 2) && (fractionOnDim > 1) );
    ...
    for (int i=0; i<meshExtent.size(); i++) 
      {	
   ...
   */
  bool
  getSlice(Extent meshExtent, int whichDim, bool rankZeroOnly, 
	   Offset& off, Extent& ext, std::ostringstream& s)
  {
    ...
    //Timer sliceTime(s.str().c_str(), m_MPIRank);
   ...
   */
  void
  slice(Series& series, MeshRecordComponent& rho, int whichDim, bool rankZeroOnly)
  {
    /*
    if ( rankZeroOnly && m_MPIRank )
      return;
    ...
    if ( whichDim < 0 ) return;

    Extent meshExtent = rho.getExtent();

    if ( whichDim >= meshExtent.size() ) return;
  ...
  /*
   * Handles 1D mesh read
   * @param series     openPMD series
   * @param rho        a mesh
   */
  /*
  void 
  slice1D( Series& series, MeshRecordComponent& rho )
  {    
    Extent rExtent = rho.getExtent();

    auto blob = rExtent[0]/(10*m_MPISize);
    if (0 == blob)
      return;

    auto start = rExtent[0]/4;

    std::ostringstream s;
    s << "slice retrievel time, ["<<start<<" + "<< (blob * m_MPISize) <<"] ";
  ...
  */
  ...
  /*
   * Handles 2D mesh read
   * @param series     openPMD series
   * @param rho        a mesh
   */
  /*
  void 
  slice2D( Series& series, MeshRecordComponent& rho )
  {    
    if ( m_Pattern % 3 == 0) {
