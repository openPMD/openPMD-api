...
public:
  ...
  void storeMesh(Series& series, int step, const std::string& fieldName, const std::string& compName);
 ...
 */
AbstractPattern::AbstractPattern(const TestInput& input)
    :m_Input(input)
{}
...
void AbstractPattern::store(Series& series, int step)
  {
    ...
    for (int i=0; i<m_GlobalMesh.size(); i++) 
    {
   ...
   */
  void
  AbstractPattern::storeMesh(Series& series, int step, const std::string& fieldName, const std::string& compName)
  {
   ...
   */
  void
  AbstractPattern::storeParticles( ParticleSpecies& currSpecies,  int& step )
  {
	...
	//std::cout<<m_Input.m_MPIRank<<"... got p: "<<offset<<", "<<count<<std::endl;
        if (count > 0) {
...
void AbstractPattern::PrintMe()
  {
	...
	m <<m_MinBlock[i]<<" ";
      ...
      std::cout<<"MPI Size: "<<m_Input.m_MPISize<<"  mesh/particle ratio="<<m_Input.m_Ratio;
      std::cout<<" UnBalance? "<<m_Input.m_Unbalance<<"  steps="<<m_Input.m_Steps;
    ...
    for ( int i=0; i< m_InRankMeshLayout.size(); i++ ) {
...
bool TwoDimPattern::setLayOut(int step)  {
    ...
    Extent patchGrid = { m_GlobalUnitMesh[0]/m_PatchUnitMesh[0],
	...
	for ( auto i=0; i<unitExtent[0]; i++ ) 
	  for ( auto j=0; j<unitExtent[1]; j++ ) 
	    {
 ...
 * @param result: return
 */
inline void TwoDimPattern::coordinate(unsigned long idx, const Extent& grid, Offset& result)
{
  auto yy = idx % grid[1];
 ...
 * @param result: return
 */
inline void ThreeDimPattern::coordinate(unsigned long idx, const Extent& grid, Offset& result)
{
  auto zz = idx % grid[2];
