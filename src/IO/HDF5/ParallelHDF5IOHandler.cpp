...
void
ParallelHDF5IOHandlerImpl::createFile(Writable* writable,
...
void
ParallelHDF5IOHandlerImpl::createPath(Writable* writable,
                                      std::map< std::string, Argument > const& parameters)
{
    if( !writable->written )
    {
...
void
ParallelHDF5IOHandlerImpl::createDataset(Writable* writable,
                                 std::map< std::string, Argument > const& parameters)
{
    if( !writable->written )
    {
...
void
ParallelHDF5IOHandlerImpl::openFile(Writable* writable,
                                    std::map< std::string, Argument > const& parameters)
{
    using namespace boost::filesystem;
    path dir(m_handler->directory);
...
void
ParallelHDF5IOHandlerImpl::openPath(Writable* writable,
                                    std::map< std::string, Argument > const& parameters)
{
    auto res = m_fileIDs.find(writable);
    if( res == m_fileIDs.end() )
...
void
ParallelHDF5IOHandlerImpl::openDataset(Writable* writable,
...
void
ParallelHDF5IOHandlerImpl::deleteFile(Writable* writable,
                                      std::map< std::string, Argument > const& parameters)
{
    if( m_handler->accessType == AccessType::READ_ONLY )
...
void
ParallelHDF5IOHandlerImpl::deletePath(Writable* writable,
                                      std::map< std::string, Argument > const& parameters)
{
    if( m_handler->accessType == AccessType::READ_ONLY )
...
void
ParallelHDF5IOHandlerImpl::deleteDataset(Writable* writable,
                                         std::map< std::string, Argument > const& parameters)
{
    if( m_handler->accessType == AccessType::READ_ONLY )
        throw std::runtime_error("Deleting a path in a file opened as read only is not possible.");
    ...
    if( writable->written )
    {
        ...
        herr_t status = H5Ldelete(node_id,
                                  name.c_str(),
                                  H5P_DEFAULT);
        ASSERT(status == 0, "Internal error: Failed to delete HDF5 dataset");
...
void
ParallelHDF5IOHandlerImpl::deleteAttribute(Writable* writable,
                                           std::map< std::string, Argument > const& parameters)
{
    if( m_handler->accessType == AccessType::READ_ONLY )
        throw std::runtime_error("Deleting a path in a file opened as read only is not possible.");
...
void
ParallelHDF5IOHandlerImpl::writeDataset(Writable* writable,
                                        std::map< std::string, Argument > const& parameters)
{
    auto res = m_fileIDs.find(writable);
    if( res == m_fileIDs.end() )
...
void
ParallelHDF5IOHandlerImpl::writeAttribute(Writable* writable,
                                  std::map< std::string, Argument > const& parameters)
{
    auto res = m_fileIDs.find(writable);
    if( res == m_fileIDs.end() )
...
void
ParallelHDF5IOHandlerImpl::readDataset(Writable* writable,
                                       std::map< std::string, Argument > & parameters)
{
    auto res = m_fileIDs.find(writable);
    if( res == m_fileIDs.end() )
...
void
ParallelHDF5IOHandlerImpl::readAttribute(Writable* writable,
                                         std::map< std::string, Argument >& parameters)
{
    auto res = m_fileIDs.find(writable);
    if( res == m_fileIDs.end() )
...
void
ParallelHDF5IOHandlerImpl::listPaths(Writable* writable,
                                     std::map< std::string, Argument > & parameters)
{
    auto res = m_fileIDs.find(writable);
    if( res == m_fileIDs.end() )
...
void
ParallelHDF5IOHandlerImpl::listDatasets(Writable* writable,
                                        std::map< std::string, Argument >& parameters)
{
    auto res = m_fileIDs.find(writable);
    if( res == m_fileIDs.end() )
...
void ParallelHDF5IOHandlerImpl::listAttributes(Writable* writable,
                                       std::map< std::string, Argument >& parameters)
{
    auto res = m_fileIDs.find(writable);
    if( res == m_fileIDs.end() )
