...
main()
{
    ...
    {
        auto f = io::Series("working/directory/2D_simData.h5", io::Access::CREATE);
...

        {
            ...
            reference.setComment("Modifications to a copied iteration refer to the same iteration");
        }
        ...
        cur_it.meshes["generic_2D_field"].setUnitDimension({{io::UnitDimension::L, -3},
                                                            {io::UnitDimension::M, 1}});

        {
            // copies of objects are handles/references to the same underlying object
...

        {
            ...
            electrons["displacement"].setUnitDimension({{io::UnitDimension::M, 1}});
            electrons["displacement"]["x"].setUnitSI(1e-6);
            electrons.erase("displacement");
            electrons["weighting"][io::RecordComponent::SCALAR].makeConstant(1.e-5);
        }
        ...
        std::shared_ptr<double> partial_mesh(new double[5], [](double const *p) {
            delete[] p;
            p = nullptr;
        });
  ...
  "adios2": {
    "dataset": {
      "operators": [
        ...
        io::Extent mpiDims{4};
        std::shared_ptr<float> partial_particlePos(new float[2], [](float const *p) {
            delete[] p;
            p = nullptr;
        });
        ...
        std::shared_ptr<uint64_t> partial_particleOff(new uint64_t[2], [](uint64_t const *p) {
            delete[] p;
            p = nullptr;
        });
        ...
        electrons.particlePatches["numParticles"][io::RecordComponent::SCALAR].resetDataset(dset);
        electrons.particlePatches["numParticlesOffset"][io::RecordComponent::SCALAR].resetDataset(dset);

        dset = io::Dataset(io::Datatype::FLOAT, {2});
        electrons.particlePatches["offset"].setUnitDimension({{io::UnitDimension::L, 1}});
        electrons.particlePatches["offset"]["x"].resetDataset(dset);
        electrons.particlePatches["extent"].setUnitDimension({{io::UnitDimension::L, 1}});
        ...
        for (uint64_t i = 0u; i < 2u; ++i) {
            ...
            electrons.particlePatches["numParticles"][io::RecordComponent::SCALAR].store(i, numParticles);
            electrons.particlePatches["numParticlesOffset"][io::RecordComponent::SCALAR].store(i, numParticlesOffset);

            electrons.particlePatches["offset"]["x"].store(i, particle_position[numParticlesOffset]);
            electrons.particlePatches["extent"]["x"].store(i, particle_position[numParticlesOffset + numParticles - 1] -
