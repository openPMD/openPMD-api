...
class pipe:
    ...
    def run(self):
        if self.comm.size == 1:
            ...
            sys.stdout.flush()
            inseries = io.Series(
                self.infile, io.Access.read_only, self.comm, self.inconfig
            )
    ...
    def __copy(self, src, dest, current_path="/data/"):
        ...
        if isinstance(src, io.Series):
            ...
            if src.empty:
                ...
                self.loads.append(
                    deferred_load(src, span, local_chunk.offset, local_chunk.extent)
                )
...
def main():
    ...
    if HAVE_MPI:
        run_pipe = pipe(
            args.infile, args.outfile, args.inconfig, args.outconfig, MPI.COMM_WORLD
        )
    else:
        run_pipe = pipe(
            args.infile,
