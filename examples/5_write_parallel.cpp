...
int main(int argc, char *argv[])
{
  ...
  {
    if( 0 == mpi_rank )
      cout << "Opened a C++ scope to ensure correct object destruction prior "
              "to MPI_Finalize()\n";
    ...
    *local_data = global_data[mpi_rank];
    ...
    Series series = Series::create("../samples/5_parallel_write.h5",
                                   MPI_COMM_WORLD,
                                   AccessType::CREATE);
    if( 0 == mpi_rank )
      cout << "Created an empty series in parallel with " << mpi_size << " MPI ranks\n";
